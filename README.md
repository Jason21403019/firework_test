...
「幸福花火轉一夏」是聯合新聞網舉辦的互動式運勢轉運活動，讓用戶透過轉運獲得不同的煙火結果，並有機會參與抽獎活動。
...
- 首次轉運獲得 LINE Points 5點
- 里程碑成就系統
- 抽獎活動參與資格

## 安全流程

1. **流程驗證**: 用戶必須透過正確流程（點擊轉運按鈕 → 登入 → 驗證）
2. **令牌管理**:
   - 流程安全令牌（10分鐘有效期）
   - CSRF 令牌（5分鐘有效期）
   - Turnstile 驗證令牌
...
- `saveUserData.php`: 保存轉運資料
- `checkPlayStatus.php`: 檢查轉運狀態
