...
轉運卡片 JS 檔案已內建樣式，如需自訂可以覆寫以下 CSS 類別：

```css
.divination-card {
	/* 卡片容器 */
}
.divination-card__inner {
	/* 卡片內容 */
}
.divination-card__img {
	/* 卡片圖片 */
}
.divination-card__title {
	/* 卡片標題 */
}
.divination-card__desc {
	/* 卡片描述 */
}
.divination-first-popup {
	/* 首次完成彈窗 */
}
```
